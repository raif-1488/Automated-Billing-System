...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validation purpose
    $parseNumber = $phoneUtil->parse($contact,$region);
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO consumers (consumer_id, name, email, contact, address, password) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$consumer_id, $name, $email, $contact, $address, $hashedPassword]);
            
            $success = "Consumer added successfully!<br><strong>Email:</strong> {$email}<br><strong>Password:</strong> {$randomPassword}";
            sendCredentials($email, $name, $randomPassword); //to send user their credentials
    ...
    <!-- <script>
        ...
        const iti = window.intlTelInput(input, {
            ...
            geoIpLookup: function (callback) {
                fetch("https://ipinfo.io/json?token=<7035885f651849>")
                    .then((resp) => resp.json())
